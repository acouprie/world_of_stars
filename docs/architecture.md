...
```
Rails (rendu serveur)
├── Turbo Frames      → navigation partielle, formulaires, pages bâtiments
├── Turbo Streams     → push live (ressources, timers, alertes)
├── Stimulus          → interactions légères, comportements JS ciblés
└── React islands     → composants Canvas/WebGL autonomes
...
Kamal est l'outil de déploiement officiel de l'écosystème Rails (créé par Basecamp). Il orchestre Docker sur VPS sans la complexité de Kubernetes.

```yaml
# config/deploy.yml (simplifié)
service: world-of-stars
image: your-registry/world-of-stars
