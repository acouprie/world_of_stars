...
_(Le radar occupe le 16e emplacement — l'anneau orbital — distinct des 15 slots sol.)_
...
_Niveau 10 requis : Centre de Commandement niv 11. C'est un objectif end game._
...
**Rôle** : seul lieu de production des unités terrestres. Monter le niveau **réduit le temps de production** de toutes les unités. Files de production parallèles supplémentaires débloquées par **technologie** (pas par le niveau du bâtiment).

_Note : le training_camp ne débloque aucun type d'unité — c'est le military_camp qui s'en charge._

| Niv | Métal  | Nourriture | Thorium | Énergie conso. | Temps   |
| --- | ------ | ---------- | ------- | -------------- | ------- |
| 1   | 250    | 250        | 100     | 20             | 45s     |
| 2   | 500    | 500        | 128     | 35             | 1m 26s  |
| 3   | 1 000  | 1 000      | 400     | 50             | 2m 42s  |
| 4   | 1 344  | 1 344      | 800     | 65             | 5m 08s  |
| 5   | 2 688  | 4 000      | 1 024   | 80             | 18m 34s |
| 6   | 5 376  | 5 376      | 2 048   | 95             | 33m 50s |
| 7   | 10 752 | 10 752     | 4 092   | 110            | 45m 17s |
| 8   | 21 504 | 21 504     | 8 192   | 125            | 1h 07m  |
| 9   | 43 008 | 43 008     | 16 384  | 135            | 2h 07m  |
| 10  | 86 016 | 86 016     | 32 768  | 150            | 4h 00m  |
...
**Rôle** : débloque les types d'unités terrestres selon son niveau. Ne produit pas d'unités (la production est dans le training_camp). Certaines unités peuvent avoir des **dépendances multiples** — par exemple, le scientifique nécessite un certain niveau de military_camp **et** de research_lab.

_Le calendrier de déblocage par niveau sera défini dans `unit_reference.md` avec le roster complet._
...
**Rôle** : le bunker permet au joueur de **mettre à l'abri consciemment** des unités et des ressources **avant** un combat. C'est une action stratégique, pas un repli automatique. Les unités abritées sont **immunisées** et ne participent pas au combat. Le joueur peut préférer se faire piller plutôt que de perdre des troupes.

- **1 unité = 1 slot** dans le bunker, quel que soit le type (combattant, transport, scientifique...).
- La capacité `resources` est **globale** : répartie librement entre métal, nourriture et thorium.
- La capacité `soldiers` définit le nombre maximum d'unités protégées.
...
- **Réduction du temps de production** du training_camp : formule exacte par niveau à définir (ex : −X% par niveau).
- **Files de production parallèles** : débloquées par technologie — le nombre de files et la technologie requise restent à définir.
- **Calendrier de déblocage des unités** par le military_camp : à définir avec le roster dans `unit_reference.md`.
- Coût d'utilisation du portail quantique (thorium par trajet) : toujours non tranché (voir questions ouvertes GDD).
- **Capacité de garnison** (military_camp) : amélioration future envisagée (nombre maximum d'unités stationnées par planète selon le niveau du camp). Reportée.
...
_Document vivant v1.1 — à maintenir en parallèle de buildings.rb_
