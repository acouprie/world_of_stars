...
> Complément au `game_design.md`, `combat_reference.md`, `building_reference.md`, `unit_reference.md` et `starships_reference.md`
...
8. [Technologies spatiales](#8-technologies-spatiales)
9. [Évolutions futures](#9-évolutions-futures)
10. [Correspondance World of Stargate](#10-correspondance-world-of-stargate)
11. [Structure de données](#11-structure-de-données)
...
- L'ajout de files de recherche parallèles n'est **pas** dans le périmètre initial (évolution future possible — voir section 9).
...
| **Cartographie stellaire** | Exploration | + gains XP & ressources en exploration, − risque de pertes | 10      | —                        |
...
Technologies dont les fonctionnalités ne sont pas encore implémentées, mais dont le design est figé.

| Technologie         | Catégorie    | Effet                                | Niv max | Cross-dépendance                             |
| ------------------- | ------------ | ------------------------------------ | ------- | -------------------------------------------- |
| **Cœur de thorium** | Énergie      | + production d'énergie des centrales | 10      | Exploration 3, Conversion énergétique 5      |
| **Renseignement**   | Espionnage   | + niveau d'espionnage                | 10      | —                                            |
| **Colonisation**    | Colonisation | +1 planète colonisable par niveau    | 2       | Exploration 2 (niv 1), Exploration 4 (niv 2) |

---

## 8. Technologies spatiales

Technologies spécifiques pour la construction et l'amélioration des vaisseaux. Ces technologies sont **requises** pour débloquer certains vaisseaux (cf. `starships_reference.md`).

| Technologie                  | Catégorie  | Effet                                                            | Niv max | Cross-dépendance                               |
| ---------------------------- | ---------- | ---------------------------------------------------------------- | ------- | ---------------------------------------------- |
| **Générateur d'hyperespace** | Propulsion | Permet le voyage en hyperespace pour les vaisseaux               | 10      | Réacteur inertiel 3                            |
| **Réacteur à Naquadah**      | Propulsion | + vitesse et efficacité énergétique des vaisseaux                | 10      | Générateur d'hyperespace 2                     |
| **Technologie à Ions**       | Propulsion | Améliore la propulsion ionique des vaisseaux                     | 14      | Réacteur inertiel 5                            |
| **Technologie Plasma**       | Propulsion | Améliore la propulsion plasma des vaisseaux                      | 8       | Réacteur à Naquadah 3                          |
| **Réacteur inertiel**        | Propulsion | Base pour les systèmes de propulsion avancés                     | 10      | —                                              |
| **Technologie Cristal**      | Matériaux  | Permet l'utilisation de cristaux pour les vaisseaux et bâtiments | 14      | Forage cristallin 3                            |
| **Technologie Naquadria**    | Matériaux  | Utilisation du Naquadria pour les vaisseaux avancés              | 8       | Réacteur à Naquadah 5, Technologie Cristal 6   |
| **Réacteur Asgard**          | Propulsion | Technologie de propulsion ultra-avancée                          | 12      | Technologie à Ions 8, Technologie Cristal 8    |
| **Maîtrise de l'énergie**    | Énergie    | Optimisation de la consommation énergétique des vaisseaux        | 10      | Conversion énergétique 5                       |
| **Technologie Hyperespace**  | Propulsion | Permet des voyages ultra-rapides en hyperespace                  | 10      | Générateur d'hyperespace 8, Réacteur Asgard 10 |

---

## 9. Évolutions futures

- Ajout de **files de recherche parallèles** (via une technologie ou un bâtiment dédié).
- **Bonus de combat ciblés** par type d'unité (ex : +ATQ pour les unités de combat rapproché uniquement).
- **Technologies de faction** : chaque faction pourrait avoir des technologies uniques (à développer après la v1).
- **Recherches temporaires** : technologies à durée limitée (ex : bonus de vitesse pour une mission spécifique).
...
## 10. Correspondance World of Stargate
...
module Technologies
  ...
  REGISTRY = {
    ...
    generateur_hyperespace: {
      category: :propulsion, scope: :initial, max_level: 10,
      requires: { research_lab: 3, reacteur_inertiel: 3 },
      effect: { type: :hyperespace_unlock }
    },
    reacteur_naquadah: {
      category: :propulsion, scope: :initial, max_level: 10,
      requires: { research_lab: 3, generateur_hyperespace: 2 },
      effect: { type: :vitesse_bonus, per_level: 0.05 }
    },
    technologie_ions: {
      category: :propulsion, scope: :initial, max_level: 14,
      requires: { research_lab: 3, reacteur_inertiel: 5 },
      effect: { type: :vitesse_bonus, per_level: 0.04 }
    },
    technologie_plasma: {
      category: :propulsion, scope: :initial, max_level: 8,
      requires: { research_lab: 3, reacteur_naquadah: 3 },
      effect: { type: :vitesse_bonus, per_level: 0.06 }
    },
    reacteur_inertiel: {
      category: :propulsion, scope: :initial, max_level: 10,
      requires: { research_lab: 1 },
      effect: { type: :base_propulsion }
    },
    technologie_cristal: {
      category: :materials, scope: :initial, max_level: 14,
      requires: { research_lab: 3, forage_cristallin: 3 },
      effect: { type: :material_efficiency }
    },
    technologie_naquadria: {
      category: :materials, scope: :anticipated, max_level: 8,
      requires: { research_lab: 5, reacteur_naquadah: 5, technologie_cristal: 6 },
      effect: { type: :advanced_material_unlock }
    },
    reacteur_asgard: {
      category: :propulsion, scope: :anticipated, max_level: 12,
      requires: { research_lab: 8, technologie_ions: 8, technologie_cristal: 8 },
      effect: { type: :ultra_propulsion }
    },
    maitrise_energie: {
      category: :energy, scope: :initial, max_level: 10,
      requires: { research_lab: 3, conversion_energetique: 5 },
      effect: { type: :energy_efficiency, per_level: 0.03 }
    },
    technologie_hyperespace: {
      category: :propulsion, scope: :anticipated, max_level: 10,
      requires: { research_lab: 8, generateur_hyperespace: 8, reacteur_asgard: 10 },
      effect: { type: :ultra_hyperespace }
    }
  }.freeze
